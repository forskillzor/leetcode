...
void add_item(Item **items, int num, int i)
{
    Item *item = malloc(sizeof(Item));
    item->num = num;
    item->idx = i;
    HASH_ADD_INT(*items, num, item);
}

Item* find_item(Item **items, int num)
{
    Item *item = NULL;
    HASH_FIND_INT(*items, &num, item);
    return item;
}
void delete_items(Item *items)
{
    HASH_CLEAR(hh, items);
}
...
int* twoSum(int* nums, int numsSize, int target, int* returnSize)
{
    ...
    while (i < numsSize-1)
    {
        add_item(&items, nums[i], i);
        item = find_item(&items, target - nums[++i]);
        if (item == NULL)
            continue;
        if (target == (item->num + nums[i]))
        {
            res[0] = item->idx < i ? item->idx : i;
            res[1] = item->idx > i ? item->idx : i;
            delete_items(items);
            return res;
        }
    }
